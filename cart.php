...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['product_id'])) {
    ...
    // Verificar se a sessão do carrinho já existe e é um array
    ...
    if ($product) {
        // Calcular o preço com 75% de desconto
        $discounted_price = $product['price'] * 0.25;
...
function confirmRemoval() {
    return confirm('Você realmente deseja remover este item do carrinho?');
}
</script>
        ...
        <div class="container cart">
            ...
            <?php if (!empty($cart)): ?>
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>Preço com Desconto de primeira compra 75%</th>
                    ...
                    <tbody>
                        ...
                        foreach ($cart as $product_id => $item) {
                            ...
                            $product = $result->fetch_assoc();
                            if ($product) {
                                // Verifica se 'discounted_price' está definido
                                $item_discounted_price = isset($item['discounted_price']) ? $item['discounted_price'] : $product['price'] * 0.25;
                                $item_total = $item['quantity'] * $item_discounted_price;
                                $total += $item_total;
                        ?>
                        <tr>
                            ...
                            <td>R$ <?php echo number_format($item_discounted_price, 2, ',', '.'); ?></td>
                            <td>R$ <?php echo number_format($item_total, 2, ',', '.'); ?></td>
                            <td><a href="remove_item.php?id=<?php echo intval($product_id); ?>" class="btn remove" onclick="return confirmRemoval();">Remover</a></td>
                        </tr>
                        <?php
                            } else {
                                echo "<tr><td colspan='6'>Produto não encontrado.</td></tr>";
                            }
                        } ?>
                    </tbody>
                    <tfoot>
                        <tr>
                            <td colspan="5"><strong>Total com Desconto:</strong></td>
                ...
                <div class="cart-actions">
                    <a href="loadingCart.html" class="btn">Finalizar Compra</a>
